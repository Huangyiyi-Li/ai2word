    ...
    try {
        ...
        try {
            ...
            if (stylesFile) {
                ...
                // 步骤4: 安全网 —— 将所有可能的蓝色系 w:color 全部改为黑色
                stylesXml = stylesXml.replace(
                    ...
                    function(match, attrs) {
                        ...
                        if (val === '000000' || val === '010101' || val === '111111') return match;
        ...
        try {
            ...
            if (themeFile) {
                ...
                console.log('[MD2Word] ✓ theme1.xml accent 颜色已修复为黑色');
            }
        } catch (themeErr) {
    ...
    for (const el of elements) {
        switch (el.type) {
            case 'heading':
                ...
                if (headingContent.includes('<') || headingContent.includes('$') || headingContent.includes('[')) {
                    hRuns = await parseInlineContent(headingContent, docxLib);
                } else {
                    hRuns = parseTextStyles(headingContent, docxLib);
                }
